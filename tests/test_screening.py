...
def _seed_screen_runs(connection: duckdb.DuckDBPyConnection) -> None:
    connection.execute(
        ...
        values
          ('us-00000001', 'US', 'successful', timestamp '2026-01-01 00:00:00', 'profile_a', 'hash-a', null),
          ('us-00000002', 'US', 'successful', timestamp '2026-01-01 00:00:00', 'profile_b', 'hash-b', null)
        """
    )
    connection.execute(
        """
        ...
        values
          ('us-00000001', 'US', 'composite', 'AAA', 0.9, 1, '{}'),
          ('us-00000001', 'US', 'composite', 'BBB', 0.8, 2, '{}'),
          ('us-00000001', 'US', 'composite', 'CCC', 0.7, 3, '{}'),
          ('us-00000002', 'US', 'composite', 'AAA', 0.95, 1, '{}'),
          ('us-00000002', 'US', 'composite', 'DDD', 0.85, 2, '{}'),
          ('us-00000002', 'US', 'composite', 'BBB', 0.75, 3, '{}')
        """
    )
...
def test_screen_cross_references_profiles(tmp_path) -> None:
    ...
    result = run_screen(repo, Market.US, ("profile_a", "profile_b"), top_n=3)
    ...
    assert result.candidates[0].profile_ranks == {"profile_a": 1, "profile_b": 1}
    ...
    assert bbb.profile_ranks == {"profile_a": 2, "profile_b": 3}

    # DDD only in profile_b top 3
...
def test_screen_rejects_fewer_than_two_profiles(tmp_path) -> None:
    ...
    with pytest.raises(ValidationError, match="at least two"):
        run_screen(repo, Market.US, ("profile_a",), top_n=50)
...
def test_screen_rejects_duplicate_profiles(tmp_path) -> None:
    ...
    with pytest.raises(ValidationError, match="duplicate"):
        run_screen(repo, Market.US, ("profile_a", "profile_a"), top_n=50)
...
def test_screen_raises_not_found_when_profile_has_no_run(tmp_path) -> None:
    ...
    with pytest.raises(NotFoundError):
        run_screen(repo, Market.US, ("profile_a", "missing_profile"), top_n=3)
